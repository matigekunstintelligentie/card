...
float f(float x,float o){return fmod(fmod(x,o)+o,o);}
float t=1./22.;float p=0.04;
void c(float (*matrix)[COLS*3][4]){float IFS[25][10]={
...
float x_coordinate=r();float y_coordinate=r();
float color[3]={r(),r(),r()};int i=0;
int size=22;float y_offset=0.4;float x_offset=-0.15;
while(i<10000000){if(i==5000000){memmove(&IFS, &IFS2, sizeof(IFS2));
size=25;y_offset=-0.1;}float a=r();float cumulative_p=0.;
for(int z=0;z<size;z++){cumulative_p=cumulative_p+IFS[z][6];if(a<cumulative_p){
...
break;}}float zoom=0.1;matrix[(int) (f((1.0-y_coordinate*zoom+y_offset), 1.0)*ROWS*3)][(int) (f(x_coordinate*zoom*ROWS/COLS+0.5+x_offset, 1.0)*COLS*3)][0]=color[0];
matrix[(int) (f((1.0-y_coordinate*zoom+y_offset), 1.0)*ROWS*3)][(int) (f(x_coordinate*zoom*ROWS/COLS+0.5+x_offset, 1.0)*COLS*3)][1]=color[1];
matrix[(int) (f((1.0-y_coordinate*zoom+y_offset), 1.0)*ROWS*3)][(int) (f(x_coordinate*zoom*ROWS/COLS+0.5+x_offset, 1.0)*COLS*3)][2]=color[2];
matrix[(int) (f((1.0-y_coordinate*zoom+y_offset), 1.0)*ROWS*3)][(int) (f(x_coordinate*zoom*ROWS/COLS+0.5+x_offset, 1.0)*COLS*3)][3]=matrix[(int) (f((1.0-y_coordinate*zoom+y_offset), 1.0)*ROWS*3)][(int) (f(x_coordinate*zoom*ROWS/COLS+0.5+x_offset, 1.0)*COLS*3)][3]+1;
if(matrix[(int) (f((1.0-y_coordinate*zoom+y_offset), 1.0)*ROWS*3)][(int) (f(x_coordinate*zoom*ROWS/COLS+0.5+x_offset, 1.0)*COLS*3)][3]>max_frequency){
max_frequency=matrix[(int) (f((1.0-y_coordinate*zoom+y_offset), 1.0)*ROWS*3)][(int) (f(x_coordinate*zoom*ROWS/COLS+0.5+x_offset, 1.0)*COLS*3)][3];}
i=i+1;}}int main(){printf("P6 %i %i 255 ", COLS, ROWS);
float (*matrix)[COLS*3][4];matrix=malloc(ROWS*3 * sizeof *matrix);
c(matrix);for(int x=1; x < ROWS*3-1; x+=3){
