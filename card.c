...
float fmodn(float x, float o){
	return fmod(fmod(x,o) + o, o);
}

void chaos_game(float (*matrix)[COLS*3][4]){
	float IFS[25][10] = {
	...
	float x_coordinate = random_uniform();
	float y_coordinate = random_uniform();
	float color[3] = {random_uniform(),random_uniform(),random_uniform()};
	...
	while(i<10000000){
		...
		for(int z=0;z<size;z++){
			cumulative_p = cumulative_p + IFS[z][6];
			if(r<cumulative_p){
				...
				x_coordinate = x_t;
				color[0] = (color[0] + IFS[z][7])*0.5;
				color[1] = (color[1] + IFS[z][8])*0.5;
				color[2] = (color[2] + IFS[z][9])*0.5;
				break;
			}
		}
		...
		matrix[(int) (fmodn((1.0 - y_coordinate*zoom + y_offset), 1.0)*ROWS*3)][(int) (fmodn(x_coordinate*zoom*ROWS/COLS + 0.5 + x_offset, 1.0)*COLS*3)][0] = color[0];
...
int main(){
	printf("P6 %i %i 255 ", COLS, ROWS);

	float (*matrix)[COLS*3][4];

	matrix = malloc(ROWS*3 * sizeof *matrix);

	chaos_game(matrix);

     for(int x = 1; x < ROWS*3 - 1; x+=3){
       	for(int y = 1; y < COLS*3 - 1; y+=3){
       		float r = 0.0;
       		float g = 0.0;
       		float b = 0.0;
       		float freq = 0.0;
       		for(int i=-1; i<2; i++){
       			for(int j=-1; j<2; j++){
       				r = r + matrix[x+i][y+j][0];
       				g = g + matrix[x+i][y+j][1];
       				b = b + matrix[x+i][y+j][2];
       				freq = freq + matrix[x+i][y+j][3];
       			}
       		}
         	printf("%c%c%c",
                     (int) (r/9.0*pow(log(freq/9.0)/log(max_frequency),1./2.2)*255.),
                     (int) (g/9.0*pow(log(freq/9.0)/log(max_frequency),1./2.2)*255.),
                     (int) (b/9.0*pow(log(freq/9.0)/log(max_frequency),1./2.2)*255.));
         }
     }
